...
public class CheckSelfWorker implements Runnable {
    ...
    public void run() {
        ...
        final String ip = serviceIp.split(Constants.COLON)[0];
        try {
            map.put(ip, ZkNodeStateEnum.CHECKING.getMsg());
            ...
            nodeCache.getListenable().addListener(new NodeCacheListener() {
                ...
                public void nodeChanged() throws Exception {
                    ...
                    if (data.compareTo(ZkConstants.ZK_PATH_DATA) == -1) {
                        ...
                        map.put(ip, ZkNodeStateEnum.WATCHER.getMsg());
                    }
                }
            });
            ...
            if (StringUtils.isBlank(zkData) || !zkData.startsWith(ZkConstants.ZK_GUARD_PATH)) {
                map.put(ip.split(Constants.COLON)[0], ZkNodeStateEnum.DATAERROR.getMsg());
                logger.error(MonitorConstant.LOG_CHECK_SELF_WORKER+"检测"+serviceIp+"过程中，直接读取到的数据不符规范，读取数据:["+zkData+"]");
            }else{
                map.put(ip.split(Constants.COLON)[0], ZkNodeStateEnum.OK.getMsg());
                logger.info(MonitorConstant.LOG_CHECK_SELF_WORKER+"检测"+serviceIp+"数据成功");
            }
            Thread.sleep(50);//休眠50毫秒，这里处理逻辑不太好，经测试，watcher是异步的，如果不休眠，要不就监听不到，要不就close连接时保异常。暂时先这样处理。
            nodeCache.getListenable().clear();
        } catch (Exception e) {
            map.put(ip, ZkNodeStateEnum.CRASH.getMsg());
